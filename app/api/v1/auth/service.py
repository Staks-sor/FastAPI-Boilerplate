...
"""Вместо обработки try-except блоков внутри каждой функции, можно бы было 
использовать централизованный обработчик ошибок для всех исключений, связанных с токенами. 
Например, можно создать универсальный метод декодирования, который будет обработку исключений 
делать через маппинг ошибок, что сделает код менее избыточным."""
"""Методы, связанные с JWT, 
не требуют выполнения асинхронных операций, так как jwt.encode и
 jwt.decode — это синхронные операции. Сделать их асинхронными было
  бы нецелесообразно, так как это вряд ли улучшит производительность 
  (и наоборот, добавляет излишние сложности).
"""